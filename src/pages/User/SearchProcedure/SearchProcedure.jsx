                ...
                <div className={styles.searchSection}>
                    ...
                    <div className={styles.formGrid}>
                        <div className={styles.formGroup}>
                            ...
                            <select className={styles.inputControl} value={typeCompany} onChange={e => setTypeCompany(e.target.value)}>
                                <option value="">Tất cả</option>
                                {typeCompanyOptions.map((opt, i) => (
                                    <option key={i} value={opt.value}>{opt.title}</option>
                                ))}
                            </select>
                        </div>
                        <div className={styles.formGroup}>
                            <label>Tên thủ tục hành chính</label>
                            <select className={styles.inputControl} value={serviceType} onChange={e => setServiceType(e.target.value)}>
                                <option value="">Tất cả</option>
                        ...
                        <div className={styles.formGroup}>
                            <label>Từ ngày</label>
                            <input type="date" className={styles.inputControl} placeholder="dd/mm/yyyy" value={startDate} onChange={e => setStartDate(e.target.value)} />
                        </div>
                        <div className={styles.formGroup}>
                            <label>Đến ngày</label>
                            <input type="date" className={styles.inputControl} placeholder="dd/mm/yyyy" value={endDate} onChange={e => setEndDate(e.target.value)} />
                        </div>
                        ...
                        {activeTab === 0 && (
                            <>
                                <div className={styles.formGroup}>
                                    ...
                                    <input type="text" className={styles.inputControl} value={code} onChange={e => setCode(e.target.value)} />
                                </div>
                                <div className={styles.formGroup}>
                                    ...
                                    <div className={styles.captchaWrapper}>
                                        ...
                                        <button className={styles.refreshCaptcha}>
                                            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                <path d="M12 4V1L8 5L12 9V6C15.31 6 18 8.69 18 12C18 13.01 17.75 13.97 17.3 14.8L18.76 16.26C19.54 15.03 20 13.57 20 12C20 7.58 16.42 4 12 4ZM12 18C8.69 18 6 15.31 6 12C6 10.99 6.25 10.03 6.7 9.2L5.24 7.74C4.46 8.97 4 10.43 4 12C4 16.42 7.58 20 12 20V23L16 19L12 15V18Z" fill="#1e1b4b" />
                                            </svg>
                                        </button>
                                    </div>
                                </div>
                            ...
                            {(activeTab === 0 ? dataRegistered : dataDraft).length === 0 ? (
                                ...
                                (activeTab === 0 ? dataRegistered : dataDraft).map((item, index) => (
                                    <tr key={item.procedureId || index}>
                                        ...
                                        <td>{item.submissionDate ? new Date(item.submissionDate).toLocaleDateString() : '-'}</td>
                                        <td style={{ color: getProcedureStatusColor(item.status) }}>{getProcedureStatus(item.status)}</td>
                                        <td className={styles.actionCell}>
                                            {activeTab === 0 ? (
                                                <button
                                                    className={styles.textBtn}
                                                    disabled={downloadingId === item.procedureId}
                                                    onClick={() => handleDownloadFile(item.procedureId, item.title || 'ho_so')}
                                                >
                                                    {downloadingId === item.procedureId ? 'Đang tải...' : (
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="15" height="16" viewBox="0 0 15 16" fill="none">
                                                            <path d="M6.17091 14.5996H2.4567C1.43105 14.5996 0.599603 13.8161 0.599609 12.8496L0.599681 2.3496C0.599688 1.38311 1.43114 0.599609 2.45678 0.599609H10.8139C11.8396 0.599609 12.671 1.38311 12.671 2.34961V7.16211M13.5996 12.8186L11.7005 14.5996M11.7005 14.5996L9.88541 12.8991M11.7005 14.5996V10.2246M3.84983 4.09961H9.42113M3.84983 6.72461H9.42113M3.84983 9.34961H6.63548" stroke="#1B154B" stroke-width="1.2" stroke-linecap="round" stroke-linejoin="round" />
                                                        </svg>
                                                    )}
                                                </button>
                                            ) : (
                                                <button className={styles.textBtn} onClick={() => handleViewDetail(item.procedureId)}>
                                                    <svg xmlns="http://www.w3.org/2000/svg" width="15" height="16" viewBox="0 0 15 16" fill="none">
                                                        <path d="M6.17091 14.5996H2.4567C1.43105 14.5996 0.599603 13.8161 0.599609 12.8496L0.599681 2.3496C0.599688 1.38311 1.43114 0.599609 2.45678 0.599609H10.8139C11.8396 0.599609 12.671 1.38311 12.671 2.34961V7.16211M13.5996 12.8186L11.7005 14.5996M11.7005 14.5996L9.88541 12.8991M11.7005 14.5996V10.2246M3.84983 4.09961H9.42113M3.84983 6.72461H9.42113M3.84983 9.34961H6.63548" stroke="#1B154B" strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round" />
                                                    </svg>
                                                </button>
                                            )}
                                        </td>
